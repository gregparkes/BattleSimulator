...
def simulate_battle(M, terrain, max_step: int = 100, ret_frames: bool = True):
    """
    ...
    if ret_frames:
        ...
        t = _step_through_update(M, Z, max_step, teams, enemy_targets, bounds, frames)
        return frames[:t]
    else:
        t = _step_through_noframe(M, Z, max_step, teams, enemy_targets, bounds)
        return np.array(
            [np.sum(np.logical_and(M["hp"] > 0.0, M["team"] == T)) for T in teams]
        )
